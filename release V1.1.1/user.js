...
`;
    ...
    floatingMenu.style.right = '50px'; ///////////////////////////WARNING MUST CHANGE TO MORE DYNAMIC IN FUTURE UPDATES
    floatingMenu.style.backgroundColor = '#1e2124';  // Dark blue
    floatingMenu.style.padding = '20px';///////////////////////////WARNING MUST CHANGE TO MORE DYNAMIC IN FUTURE UPDATES
    ...
    */
    ...
    header.innerText = '| 早报解码器 | 内测版 | V1.1.1 |' + '\n';
    ...
    /*start of code for memory usage*/

    function getTotalLocalStorageMemoryUsage() {
        let totalMemoryUsage = 0;

        for (let i = 0; i < localStorage.length; i++) {
            const key = localStorage.key(i);
            const serializedData = JSON.stringify(localStorage.getItem(key));
            const memoryUsageInBytes = new TextEncoder().encode(serializedData).length;
            totalMemoryUsage += memoryUsageInBytes;
        }

        // Convert total memory usage to appropriate units (KB, MB, GB, etc.)
        const formattedTotalMemoryUsage = formatMemoryUsage(totalMemoryUsage);

        return formattedTotalMemoryUsage;
    }

    // Helper function to format memory usage into appropriate units
    ...
    // Update memory usage display every one second
    function updateMemoryUsage() {
        const totalMemoryUsage = getTotalLocalStorageMemoryUsage();
        const memoryUsageElement = document.getElementById('memory-usage');
        memoryUsageElement.innerText = '应用缓存 [动态更新]： ' + totalMemoryUsage + '\n';
    }
    ...
    memoryUsage.style.color = '#ffffff';  // White text
    memoryUsage.style.marginBottom = '7px';
    floatingMenu.appendChild(memoryUsage);

    // Update memory usage every one second
    setInterval(updateMemoryUsage, 1000);

    /*end of code for memory usage*/

    //define state of progress
...
action = "complete";
    ...
    const spacer = document.createTextNode('\u00A0'); // '\u00A0' represents a non-breaking space

    // Append the non-breaking space to the floatingMenu
    floatingMenu.appendChild(spacer);


    const button1 = document.createElement('button');
    button1.innerText = '提取文本资料'; // "Enable Assistance" in Chinese
    button1.style.backgroundColor = '#000000';  // Black button
    button1.style.color = '#ffffff';  // White text
    button1.style.padding = '10px';
    button1.style.border = '1px solid #00008b';  // Dark blue border
    ...
    floatingMenu.appendChild(button1);
    ...
    // Create the "清除缓存" checkbox
    const checkbox3 = document.createElement('input');
    checkbox3.type = 'checkbox';
    checkbox3.id = 'checkbox3';
    const label3 = document.createElement('label');
    label3.htmlFor = 'checkbox3';
    label3.innerText = '清除缓存 [每篇一定要重开！]';
    label3.style.color = '#ffffff';  // White text
    checkbox3.checked = localStorage.getItem('checkbox3_checked') === 'true';
    checkbox3.addEventListener('change', () => {
        localStorage.setItem('checkbox3_checked', checkbox3.checked);
        if (checkbox3.checked) {
            localStorage.clear();
            alert('运行中');
            updateProgressText('缓存已清除');
        }
    });
    floatingMenu.appendChild(checkbox3);
    floatingMenu.appendChild(label3);

    const lineBreak3 = document.createElement('br');
    floatingMenu.appendChild(lineBreak3);

    // Create the "自动提交" checkbox
    const checkbox4 = document.createElement('input');
    checkbox4.type = 'checkbox';
    checkbox4.id = 'checkbox4';
    const label4 = document.createElement('label');
    label4.htmlFor = 'checkbox4';
    label4.innerText = '自动提交 [快]\u00A0';
    label4.style.color = '#ffffff';  // White text
    checkbox4.checked = localStorage.getItem('checkbox4_checked') === 'true';
    checkbox4.addEventListener('change', () => {
        localStorage.setItem('checkbox4_checked', checkbox4.checked);
        if (checkbox4.checked) {
            alert('运行中');
            updateProgressText('自动提交已开启 [防检测 - 关]');
            const interval = setInterval(() => {
                if (checkbox4 && checkbox4.checked && action === "complete") {
                  const button = document.querySelector('.btn.btn-default.btn-submit');
                  if (button) {
                    button.click();
                  } else {
    ...
    floatingMenu.appendChild(checkbox4);
    floatingMenu.appendChild(label4);
...
label5.innerText = '自动提交 [防检测]';
label5.style.color = '#ffffff';  // White text
checkbox5.checked = localStorage.getItem('checkbox5_checked') === 'true';
checkbox5.addEventListener('change', () => {
    localStorage.setItem('checkbox5_checked', checkbox5.checked);
    if (checkbox.checked) {
        alert('运行中');
        updateProgressText('自动提交已开启 [防检测 - 关]');
        const interval = setInterval(() => {
            if (checkbox5 && checkbox5.checked && action === "complete") {
              const button = document.querySelector('.btn.btn-default.btn-submit');
              if (button) {
                button.click();
              } else {
...
floatingMenu.appendChild(checkbox5);
floatingMenu.appendChild(label5);



  
    const lineBreak4 = document.createElement('br');
    floatingMenu.appendChild(lineBreak4);
