...
`;
    ...
    */
    ...
    header.innerText = '| 早报解码器 [全防]| 内测版 | V1.1.2 |' + '\n';
...
eval(autosubmit);
    ...
    floatingMenu.appendChild(spacer1);
...
// Create the "自动提交" checkbox
//locked on check by default
const checkbox5 = document.createElement('input');
checkbox5.type = 'checkbox';
checkbox5.id = 'checkbox5';
const label5 = document.createElement('label');
label5.htmlFor = 'checkbox5';
label5.innerText = '自动提交';
label5.style.color = '#ffffff';  // White text
checkbox5.checked = localStorage.getItem('checkbox5_checked') === 'true';
...
const handleCheckboxChange = () => {
    ...
    localStorage.setItem('checkbox5_checked', 'true');
};

checkbox5.addEventListener('change', () => {
    alert('自动提交不可被关闭');
    updateProgressText('自动提交不可被关闭');
    checkbox5.checked = true;  // Set the checkbox to be initially checked
checkbox5.disabled = true;  // Disable the checkbox to prevent unchecking
label5.style.pointerEvents = 'cursor';
    handleCheckboxChange();
});

// Initially set the checkbox to checked
handleCheckboxChange();
